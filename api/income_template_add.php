...
try {
    $stmt = $pdo->prepare('INSERT INTO income_templates (user_id, category_id, name, amount) VALUES (?, ?, ?, ?)');
    $stmt->execute([$uid, $categoryId, $name, $amount]);
} catch (Throwable $e) {
    json_response(['error' => 'Falta actualizar la base de datos: corré sql/upgrade_v9.sql (tabla de ingresos frecuentes no encontrada).'], 500);
}
